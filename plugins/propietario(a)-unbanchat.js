...
let handler = async (m, { conn }) => {
  if (!(m.chat in global.db.data.chats)) return m.reply('*This chat is not registered in the database!*')
  let chat = global.db.data.chats[m.chat]
  if (!chat.isBanned) return m.reply('*This chat is not banned!!*')
  chat.isBanned = false
  m.reply(`${eg}𝙏𝙃𝙄𝙎 𝘾𝙃𝘼𝙏 𝙒𝘼𝙎 𝙐𝙉𝘽𝘼𝙉 😌\n𝙉𝙊𝙒 𝙄 𝘼𝙈 𝘼𝙑𝘼𝙄𝙇𝘼𝘽𝙇𝙀.`)
}
